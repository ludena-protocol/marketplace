...
contract CharacterNFT is Ownable, ERC721Enumerable, Pausable, AccessControl {
    ...
    event settingEvent(string eventType, string groupId, uint256[] price, uint256[2] duration, uint256 quantity, uint256 lockUp, uint8 phase);
    ...
    function setPresale(string memory _group, address[] memory tokenAddress,uint256[] memory _price, uint256[2] memory duration, uint256 _quantity, uint8 _phase) external onlyRole(MANAGERMENT_ROLE){
        ...
        require(tokenAddress.length == _price.length, "Invalid token data");
        for(uint256 i = 0; i < tokenAddress.length; i++){
            groupToPrice[_group][tokenAddress[i]] = _price[i];
        }
    ...
    function setPublicSale(address[] memory tokenAddress, uint256[] memory _price, uint256[2] memory duration, uint256 _quantity, uint8 _phase) external onlyRole(MANAGERMENT_ROLE){
        ...
        require(tokenAddress.length == _price.length, "Invalid token data");
        for(uint256 i = 0; i < tokenAddress.length; i++){
            this.setPrice(tokenAddress[i], _price[i]);
        }
    ...
    function setEvent(string memory _group, uint256[2] memory duration, uint256 _quantity, uint256 lockUpTime, uint8 _phase) external onlyRole(MANAGERMENT_ROLE){
        ...
        emit settingEvent("freeEvent", _group, price, duration, _quantity, lockUpTime, _phase);
    }
     ...
     * @param selectedToken address of token that user use to pay for minting NFT.
     * @param quantity the number of NFT user want to minted.
     */
    function buyCharacter(address selectedToken, uint256 quantity) external whenNotPaused{
        ...
        require(tokenPrice > 0, "Unsupported token");
    ...
    function whitelistBuy(address selectedToken) external isWhitelisted {
        ...
        uint256 quantityOfGroup = groupToQuantity[groupWhitelist[msg.sender]];
        require(
            tokenIdNow + quantityOfGroup - 1 < limitCharacter,
            "exeed limited number"
        );
        ...
        IERC20(selectedToken).transferFrom(
            msg.sender, 
            address(this),
            tokenPrice  * quantityOfGroup
        );

        // mint one by one
        for (uint256 i = 0; i < quantityOfGroup; i++) {
            uint256 tokenId = _tokenIdCount.current();
            _mint(msg.sender, tokenId);
            availableTime[tokenId] = block.timestamp;
            _tokenIdCount.increment();
    ...
    function getFreeCharacter() external isWhitelisted{
        ...
        uint256 fCharactorQ = freeCharacterQ[groupWhitelist[msg.sender]];

        require(
            tokenIdNow + fCharactorQ - 1 < limitCharacterFree,
            "exeed limited number"
        );
        uint256[] memory listToken = new uint256[](fCharactorQ);
        isClaimedFree[msg.sender] = true;
        // mint one by one
        for (uint256 i = 0; i < fCharactorQ; i++) {
            uint256 tokenId = _tokenIdCount.current();
            _mint(msg.sender, tokenId);
